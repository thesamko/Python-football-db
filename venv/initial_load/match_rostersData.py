...
for leag in leagues:
    ...
    cursor.execute(f'TRUNCATE TABLE {schema_name}.landing_match_rostersData')
    cursor.commit()
    friendly_league_name = leag.replace('_', ' ')
    cursor.execute(f'SELECT ld.[match_id] FROM [landingdb].{schema_name}.[landing_league_datesData] ld')
