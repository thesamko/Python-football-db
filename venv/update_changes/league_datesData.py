...
class DatesData:
    ...
    def full_load(self):
        for league in self.leagues:
            schema_name = league.replace('_', '').lower()
            self.cursor.execute(f'TRUNCATE TABLE {schema_name}.landing_league_datesData_test')
            self.cursor.commit()
            for year in self.seasons:
                ...
                with self.alchemy_connection.begin() as conn:
                    ...
                    conn.exec_driver_sql(f"SET IDENTITY_INSERT {schema_name}.landing_league_datesData_test OFF")
                    season_data_df.to_sql('landing_league_datesData_test', self.alchemy_connection, schema=schema_name,
