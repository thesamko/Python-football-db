...
class ShotsData:
    ...
    def full_load(self):
        for league in self.leagues:
            schema_name = league.replace('_', '').lower()
            self.cursor.execute(f'TRUNCATE TABLE {schema_name}.landing_player_shotsData_test')
            self.cursor.commit()
        ...
        with self.alchemy_connection.begin() as conn:
            ...
            conn.exec_driver_sql(f"SET IDENTITY_INSERT {schema_name}.landing_player_shotsData_test OFF")
