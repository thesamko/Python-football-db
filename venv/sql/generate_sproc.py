...
for leag in leagues:
    ...
    for table in all_tables:
        sproc_query = f"exec GenerateSprocScript '{league}', {table}"
        landing_cursor.execute(sproc_query)
        sproc_statement = landing_cursor.fetchone()[0]
        dev_cursor.execute(sproc_statement)
        dev_cursor.commit()
